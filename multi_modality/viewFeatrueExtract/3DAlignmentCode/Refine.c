...
// FIXME:
extern void RenderToMem(unsigned char *bmBits, unsigned char *bmColor, pVer CamVertex, pVer v, pTri t, int nv, int nt);
//extern void RenderToMem(unsigned char *bmBits, pVer CamVertex, pVer v, pTri t, int nv, int nt);
				...
				pVer vertex, pTri triangle, int NumVer, int NumTri)
{
	...
	for(i=0; i<CAMNUM; i++)
        RenderToMem(destBuff[i], NULL, CamVertex+i, vertex, triangle, NumVer, NumTri);
